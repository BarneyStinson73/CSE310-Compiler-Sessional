...
public:
    ...
    void enter_scope()
    {
        ...
        temp->parent = current;
        current = temp;
        current->scope_offset = current->parent->scope_offset;
    }
    void exit_scope()
    {
    ...
    bool sinsert(SymbolInfo *s)
    {
        bool b = current->insert(s->getName(), s->getType());
        SymbolInfo *temp = new SymbolInfo();
        temp = LookUp(s->getName());
        temp->offset = s->offset;
        temp->is_global = s->is_global;
        return b;
    }
    bool osinsert(SymbolInfo *s)
    {
        bool b = current->o_insert(s->getName(), s->getType(), s->optional);
        SymbolInfo *temp = new SymbolInfo();
        temp = LookUp(s->getName());
        temp->offset = s->offset;
        temp->is_global = s->is_global;
    ...
    int getScopeCount()
    {
        return globals::scope_table_count;
    }
};
